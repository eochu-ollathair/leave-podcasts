...
"""Guide a Mac or Linux owner through starting their own podcast brief."""
...
def install_program():
    ...
    marker = VENV / ".leave-podcasts-ready"
    if PYTHON.exists() and marker.exists() and marker.read_text().strip() == digest:
        return
    print("\nPreparing Leave the Podcasts. This can take a few minutes the first time.", flush=True)
...
def main():
    ...
    print("Leave the Podcasts — your own copy")
